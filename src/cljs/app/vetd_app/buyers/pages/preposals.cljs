...
(defn c-preposal-list-item
  ...
  (let [{:keys [pname logo rounds discounts]} product
    ...
    [:> ui/Item {:on-click
                 (fn []
                   (rf/dispatch [:dispatch-stash.push :product-detail-loaded
                                 [:do-fx
                                  {:dispatch-later
                                   [{:ms 200
                                     :dispatch [:scroll-to :product/pricing]}]}]])
                   (rf/dispatch [:b/nav-product-detail (:idstr product)]))}
                    ...
                    (if (seq preposals)
                      ...
                      [:> ui/Segment {:placeholder true}
                       [:> ui/Header {:icon true}
                        ...
                        "No estimates match your filter choices."]
                       [:div {:style {:text-align "center"
                                      :margin-top 10}}
                        "To request a new estimate, search for "
                        [:a {:style {:cursor "pointer"}
                             :onClick #(rf/dispatch [:b/nav-search])}
                         "Products & Categories."]
                        [:br] [:br]
                        "Or, simply forward any sales emails you receive using the "
                        [bc/c-external-link
                         "https://chrome.google.com/webstore/detail/vetd-sales-email-gatekeep/gpmepfmejmnhphphkcabhlhfpccaabkj"
                         "Vetd Chrome Extension for Gmail"
                         {:position "right"}]]]))]]
                [:> ui/Grid
                 [:> ui/GridRow
                  ...
                  [:> ui/GridColumn {:computer 12 :mobile 16}
                   [:> ui/Segment {:placeholder true}
                    ...
                    [:div {:style {:text-align "center"
                                   :margin-top 10}}
                     "To get started, request an estimate from the "
                     [:a {:style {:cursor "pointer"}
                          :onClick #(rf/dispatch [:b/nav-search])}
                      "Products & Categories"]
                     " page."
